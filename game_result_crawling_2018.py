...
def soup_to_dataframe(url, soup, path):
  ...
  for latten_half in range(2):
    ...
    if latten_half == 0:
      my_team_class = "top"
      opponent_team_class = "bottom"
    else:
      my_team_class = "bottom"
      opponent_team_class = "top"
    my_team = soup.find("tr", class_=my_team_class)
    result_dict["team"] = my_team.find("span").text
    opponent_team = soup.find("tr", class_=opponent_team_class)
    result_dict["opponent"] = opponent_team.find("span").text
    result_dict["place"] = game_tit.find("span", class_="place").text
    result_dict["home"] = latten_half
    result_dict["runs"] = my_team.find("td", class_="total-1").text
    result_dict["opponent_runs"] = opponent_team.find("td", class_="total-1").text
    result_dict["victory"] = 1 if result_dict["runs"] > result_dict["opponent_runs"] else 0
    result_dict["hits"] = my_team.find_all("td", class_="total-2")[0].text
    result_dict["errors"] = my_team.find_all("td", class_="total-2")[1].text
    result_dict["game_info"] = soup.find("div", class_="line-score").find("p", class_="game_info").text.replace('\n','')
    if latten_half == 0:
      batter_all_result = soup.find("table", id="tablefix_t_b").find("tbody").find_all("tr")
    else:
      batter_all_result = soup.find("table", id="tablefix_b_b").find('tbody').find_all("tr")
    batters_columns_list = [
      ...
      , 'batter7_rbi', 'batter7_run', 'batter7_steal', 'batter7_strikeout', 'batter7_four_balls', 'batter7_dead_ball', 'batter7_position']
      , ['batter8_name', 'batter8_url', 'batter8_at_bat', 'batter8_hit', 'batter8_two_hit', 'batter8_three_hit', 'batter8_homerun'
      , 'batter8_rbi', 'batter8_run', 'batter8_steal', 'batter8_strikeout', 'batter8_four_balls', 'batter8_dead_ball', 'batter8_position']
    ...
    for batter_result, batter_columns in zip(batter_1_9_result, batters_columns_list):
      td_list = batter_result.find_all("td")
      result_dict[batter_columns[0]] = td_list[2].text
      result_dict[batter_columns[1]] = 'http://npb.jp' + td_list[2].a.get("href")
      result_dict[batter_columns[2]] = td_list[3].text
      result_dict[batter_columns[3]] = td_list[5].text
      ...
      for atbat_detail in td_list[8:]:
        for atbat_aggregate, atbat_aggregate_word in zip(atbat_aggregate_list, atbat_aggregate_word_list):
          if re.search(atbat_aggregate_word, atbat_detail.text):
            result_dict[atbat_aggregate] += 1
      result_dict[batter_columns[7]] = td_list[6].text
      result_dict[batter_columns[8]] = td_list[7].text
      result_dict[batter_columns[12]] = td_list[1].text.replace('(','').replace(')','')
    ...
    for batter_result in batter_others_result:
      td_list = batter_result.find_all("td")
      result_dict[atbat_aggregate_list[0]] += int(td_list[3].text)
      result_dict[atbat_aggregate_list[1]] += int(td_list[5].text)
      result_dict[atbat_aggregate_list[2]] += int(td_list[6].text)
      result_dict[atbat_aggregate_list[3]] += int(td_list[7].text)
      for atbat_detail in td_list[8:]:
    ...
    if latten_half == 0:
      pitcher_all_result = soup.find("table", id="tablefix_t_p").find("tbody").find_all("tr")
    else:
      pitcher_all_result = soup.find("table", id="tablefix_b_p").find('tbody').find_all("tr")
    starting_pitcher_result_list = pitcher_all_result[0].find_all("td")
    result_dict["st_name"] = starting_pitcher_result_list[1].text
    result_dict["st_url"] = 'http://npb.jp' + starting_pitcher_result_list[1].a.get("href")
    ...
    for st_column, starting_pitcher_result in zip(st_columns, starting_pitcher_result_list[2:]):
      if(st_column == 'st_times'):
        ...
        result_dict[st_column] = tr.find("th").text + tr.find("td").text
        continue 
      if(st_column == 'st_times_decimal'):
        continue
      result_dict[st_column] = starting_pitcher_result.text
    ...
    for relief_pitchers_result in pitcher_all_result[2::2]:
      ...
      for re_column, relief_pitcher_result in zip(re_columns, relief_pitcher_result_list[2:]):
        if(re_column == 're_times'):
          ...
          result_dict[re_column] = tr.find("th").text + tr.find("td").text
          continue 
        if(re_column == 're_times_decimal'):
          continue
        result_dict[re_column] += int(relief_pitcher_result.text)
